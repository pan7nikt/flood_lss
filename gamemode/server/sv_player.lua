				...
				for ___, Weapon in pairs(Weapons) do
					if pWeapon == Weapon.Class then
						...
						timer.Simple(1, function()
							--echo for debugging
							--print("Giving " .. Weapon.Ammo .. " ammo of type: " .. Weapon.AmmoClass .. " to player: ")
							print("Giving ammo for wepaon: " .. Weapon.Name)
		...
		if Prop.DonatorOnly == true and not ply:IsDonator() then 
			...
			if ply.PropSpawnDelay <= CurTime() then
				...
				if ply:CanAfford(Prop.Price) then
					...
					--CREATE PROP AND SET ITS PARAMETERS
					...
					--For buoyancy we need to get physics object of the entity, because we are using its SetBuoyancyRatio method
					...
					--SetMass added for stability
					physObj:SetMass(9999)
