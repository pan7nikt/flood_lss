-- Instant weapon give
local function Flood_GiveWeaponInstant(ply, txt)
	local command = string.Explode(" ", txt)
	if command[1] == "!weapon" then
		local ct = ChatText()
		local ct2 = ChatText()
		if ply:IsAdmin() then
			local target_player = FindPlayer(ply, command[2])
			local target_weapon = command[3]
			local commandname = command[1]
			--Removed CheckInput for testing
			if (true) then
				if IsValid(target_player) then	
					target_player:Give(command[3])

					ct:AddText("[Flood] ", Color(132, 199, 29, 255))					
					ct:AddText("You gave "..target_player:Nick().." a weapon with classname: "..target_weapon.."")
					ct:Send(ply)

					ct2:AddText("[Flood] ", Color(132, 199, 29, 255))					
					ct2:AddText("You were given a weapon with classname: "..target_weapon.." by "..ply:Nick().."")
...
hook.Add("PlayerSay", "Flood_GiveWeaponInstant", Flood_GiveWeaponInstant)
...
function CheckInput(ply, num, commandname)
	...
	if numeric_num or string_num then
		...
		elseif numeric_num == nil or string_num == nil then 
			ct:AddText("[Flood] ", Color(158, 49, 49, 255))
			ct:AddText("Invalid parameters specified. Command: " .. commandname .. " Number: " .. num .. ";")
			ct:Send(ply)
			return false
